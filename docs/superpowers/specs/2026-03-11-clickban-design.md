...
- Hardcoded space ID extracted from: `https://app.clickup.com/9016771227/v/s/90165823077`
- Space ID: `90165823077`, Team ID: `9016771227`
...
3. **Time range** — enter start and end times (e.g., "10:30am to now")
...
1. **Startup:** Load token from env → fetch space details → fetch all lists in space → fetch all tasks recursively per list → resolve leaf tasks → render kanban
...
| `GET /space/{space_id}/list`                | Get all lists (via folders)      |
