...
func TestRenderPreview_ContainsCustomItemName(t *testing.T) {
	task := api.Task{
		Name:       "Typed task",
		Status:     api.Status{Status: "open"},
		CustomItem: &api.CustomItem{ID: 1, Name: "Bug"},
	}
	result := RenderPreview(task, 60, 20)
	if !strings.Contains(result, "Bug") {
		t.Errorf("expected preview to contain custom item name 'Bug', got:\n%s", result)
	}
}
